...
namespace fs {
    ...
    template <typename T> inline auto rtnWithErrLog(T rtn, std::string log) { geode::log::error("{}", log); return rtn; }
    inline auto exists(path path) {
        return geode::cocos::fileExistsInSearchPaths(path.string().c_str());
    }
    inline auto read(path path) {
        unsigned long file_size = 0;
        auto buffer = cocos2d::CCFileUtils::sharedFileUtils()->getFileData(path.string().c_str(), "rb", &file_size);
